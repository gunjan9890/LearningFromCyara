...
from outbound.bridges.outbound_base import OutboundBase
...
class ZoomOutbound:
    ...
    def _handle_cookie_popup(self) -> None:
        ...
        :return:
        """
        if SeleniumTestBase.is_present(self.close_cookie_button):
            print("Cookie Popup was found. Will click on the Close Button")
            SeleniumTestBase.get_element(self.close_cookie_button).click()
    ...
    def login_bridge_as_host(self, bridge_url: str, username: str, password: str):
        ...
        print(f"Opened Bridge URL [{bridge_url}]")
        ...
        if SeleniumTestBase.is_present(terms_conditions_page.accept_terms_button):
            print(f"On Terms and condition page, Accept Terms button was found.")
            SeleniumTestBase.get_element(terms_conditions_page.accept_terms_button).click()
            print(f"Clicked on Accept Terms Button")
            time.sleep(2)
        ...
        print(f"Clicked on Sign Button")
        time.sleep(2)
    ...
    def start_meeting(self):
        ...
        :return:
        """
        # wait for the Join Dialog / Join Audio button is visible
        meeting_page = ZoomMeetingPage()
        wait = Wait.WebDriverWait(SeleniumTestBase.get_driver(), timeout=60)
        wait.until(EC.visibility_of_element_located(meeting_page.join_audio_dialog))
        self._handle_cookie_popup()

        print(f"HOST HAS NOW SUCCESSFULLY STARTED THE CONFERENCE & IS CONNECTED TO AUDIO")

    def connect_call(self, country_code: str, phone_number: str, country_name: str = "",
        ...
        """
        meeting_page = ZoomMeetingPage()
        SeleniumTestBase.get_element(meeting_page.participants_button).click()
        print(f"The Host Clicked on Participants Button")

        SeleniumTestBase.get_element(meeting_page.call_me_tab).click()
        print(f"The Host Clicked on Call Me Button")
        SeleniumTestBase.get_element(meeting_page.call_me_country_div).click()
        print(f"The Host Clicked on Country List")
        ...
        while counter < timeout_seconds:
            ...
            if "calling" in meeting_page.get_call_connection_status().lower():
                print("The Host Dialed out the phone number to connect audio")

            if "ringing" in meeting_page.get_call_connection_status().lower():
                print("The phone number is ringing")

            if "busy" in meeting_page.get_call_connection_status().lower():
                print("The phone number is busy to connect audio")
                return False
            ...
            time.sleep(1)
            counter = counter + 1
            print(f"Waited {counter} seconds")
        print(
        ...
        print(f"Closed the Call out dialog")
        time.sleep(2)
        return False
    ...
    def close_bridge(self):
        ...
        meeting_page = ZoomMeetingPage()
        self._handle_cookie_popup()
        print("Checking the End Button...")
        time.sleep(2)
        print(f"End Button is present ? : {SeleniumTestBase.is_present(meeting_page.end_button)}")
        if not SeleniumTestBase.get_element(meeting_page.end_button).is_displayed():
            print("The End Button is not visibile directly")
            print("Will have to move the mouse")
            time.sleep(1)
            actions = action_chains.ActionChains(SeleniumTestBase.get_driver())
            actions.move_to_element(SeleniumTestBase.get_element(meeting_page.video_layout)).perform()
            print("Mouse Move is completed")
        SeleniumTestBase.get_element(meeting_page.end_button).click()
        print(f"Host clicked on End Meeting button")
        SeleniumTestBase.get_element(meeting_page.end_meeting_for_all_button).click()
        time.sleep(2)
        print(f"Host clicked on End Meeting for all button")
        # remove the original saved video
    ...
    def test_bridge_call_quality(self, config: dict):
        ...
        try:
            ...
            if flag:
                ...
                print(f"Waiting for the Prompter to play enough Audio [{prompter_duration_seconds}]")
                for i in range(0, int(prompter_duration_seconds / 60)):
                    time.sleep(55)
                    # ScreenCapture.capture_screen(self.page)
                    print(f"{(i + 1)} minutes over")
...
class ZoomMeetingPage():
    def __init__(self):
        ...
        self.end_button: tuple[str, str] = ("xpath", "//button[text()='End']")
    ...
    def unmute(self):
        if SeleniumTestBase.is_present(self.join_audio_button):
            if "unmute" in SeleniumTestBase.get_element(self.join_audio_button).get_attribute("aria-label"):
